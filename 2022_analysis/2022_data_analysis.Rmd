...
gun_cases %>%
  mutate(CHARGE_SUB_CATEGORY = case_when(
    endsWith(CHARGE_OFFENSE_TITLE, "WEAPON") ~ "UUW"
  ))
...
Question: bond type for uuw and drug cases

# Drug and gun cases by bond type over time
```{r}
sa_init_month <- sa_init
...
sa_init_month %>%
  filter(OFFENSE_CATEGORY == c("Narcotics", "UUW - Unlawful Use of Weapon"))
  group_by(RECEIVED_DATE, OFFENSE_CATEGORY, BOND_TYPE_CURRENT) %>%
  summarize(n()) %>%
  ggplot(
    mapping = aes(
      x = RECEIVED_DATE,
